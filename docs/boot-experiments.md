...
| Скрипт | Назначение |
|--------|------------|
| `scripts/build-e2d-boot-experiment.sh` | Достаёт из e2d `.upk` `fastboot` / `bootargs` / `baseparam` / `kernel` (≤15 MiB) → `firmware/e2d/boot-experiment/` |
| `scripts/flash-e2d-boot-experiment.sh` | ISP write по STAGE; `rollback` → сток Cyta boot |
| `scripts/advca-imghead-parse.py` | Парсер ImgHead v2 + env CRC |
| `scripts/build-hybrid-bootargs.sh` | Cyta ADVCA header + same-length правка env + CRC |
| `scripts/build-fbldata-marker.sh` | Маркер `androidboot.fbldata=smoke1` в env `fbldata` |
...
| | |
|--|--|
| Сборка | `./scripts/build-hybrid-bootargs.sh` → `bootargs-hybrid-smoke.img` |
...
| | |
|--|--|
| Сборка | `./scripts/build-fbldata-marker.sh` |
...
```bash
YES=1 DISK=diskN sudo -E ./scripts/flash-e2d-boot-experiment.sh rollback
# или точечно:
...
./scripts/build-e2d-boot-experiment.sh
./scripts/build-hybrid-bootargs.sh
./scripts/build-fbldata-marker.sh
./scripts/advca-imghead-parse.py firmware/cyta/extracted/partitions/bootargs.img

# 1) Socket ~7.8G → diskN (Terminal.app)
YES=1 DISK=diskN STAGE=1 sudo -E ./scripts/flash-e2d-boot-experiment.sh
# UART → Android?
...
- [flash.md](flash.md) — ISP custom Android  
- [linux-install.md](linux-install.md) — e2d MENU/SD (generic путь)  
- [inventory.md](inventory.md) — `blkdevparts`  
- [uart.md](uart.md) — захват лога  
